...
/// A canned OpenAI chat-completion response. The content is JSON for `Draft`;
/// the single token logprob of -0.1 -> confidence ~0.905.
...
/// Serve `requests` HTTP exchanges on a kernel-assigned port, then stop.
fn serve() -> String {
    let listener = TcpListener::bind("127.0.0.1:0").unwrap();
    let addr = listener.local_addr().unwrap();
    thread::spawn(move || {
        listener.set_nonblocking(false).ok();
        // Two requests: one model call, one GitHub call.
        for _ in 0..2 {
...
fn round_trips_over_http() {
    ...
    // The model output flowed through async-openai; the GitHub call through
    // reqwest; the task unwrapped both and returned the PR url.
